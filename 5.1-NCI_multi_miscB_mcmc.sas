...
/* 4) Look at intakes on a given day */
	...
	%let daily_list  = vf otherfoods pfab water mufa pufa sfa freesugars sodium energy folate magnesium fibers potassium ;
	...
	ods select none;
		proc freq data=preNCI ;
		format &episo_list. &daily_list. zerofmt. ; 
		table &episo_list. &daily_list. ;
...
run;
 ...
 /********************************************************************/
 /*   Dietary constituents #16 - folate: min. amount and lambda      */
 /********************************************************************/
...
data _temp16;
set inboxcox(keep=adm_rno bsw0 folate seq2 r24_weekend agec_71plus );
if (folate > 0) then output;
run;
...
run;
 ...
 /********************************************************************/
 /*   Dietary constituents #17 - magnesium: min. amount and lambda   */
 /********************************************************************/
...
data _temp17;
set inboxcox(keep=adm_rno bsw0 magnesium seq2 r24_weekend agec_71plus );
if (magnesium > 0) then output;
run;
...
run;
 ...
 /********************************************************************/
 /*   Dietary constituents #18 - fibers: min. amount and lambda      */
 /********************************************************************/
...
var fibers ;
where fibers > 0;
output out=_min18(keep=minamount) min=minamount ;
run;
...
data _temp18;
set inboxcox(keep=adm_rno bsw0 fibers seq2 r24_weekend agec_71plus );
if (fibers > 0) then output;
run;
...
run;
 ...
 /********************************************************************/
 /*   Dietary constituents #19 - potassium: min. amount and lambda   */
 /********************************************************************/
...
var potassium ;
where potassium > 0;
output out=_min19(keep=minamount) min=minamount ;
run;
...
data _temp19;
set inboxcox(keep=adm_rno bsw0 potassium seq2 r24_weekend agec_71plus );
if (potassium > 0) then output;
run;
 ...
 * Call the <std_cov_boxcox24hr_conday_minamt> macro to standardize data and continuous covariates, if any; 
 %std_cov_boxcox24hr_conday_minamt(
  ...
  rec24hr_daily_vars         = vf otherfoods pfab water mufa pufa sfa freesugars sodium energy folate magnesium fibers potassium, 
...
* Call the <multivar_mcmc> macro to fit the measurement error model; 
 ...
 stdbc_freesugars  stdbc_sodium  stdbc_energy  stdbc_folate  stdbc_magnesium  stdbc_fibers  stdbc_potassium, 
  ...
  set_seed_mcmc               = 42941, 
  set_number_mcmc_iterations  = 10000,
  set_number_burn_iterations  = 4000,
  set_thin                    = 12,
  ...
  optional_iml_store_names    = constant1 seq2 r24_weekend agec_71plus
  								tran_paramindex tran_lambda tran_center tran_scale
  								minamount 
  ); 
...
* Save lambdas (Box-Cox transformation lambda values) and minimum amount data ;
	...
	* indicate replicate number, current stratum value, variable name;
	...
	if _N_=16 then varname = "folate" ;
	if _N_=17 then varname = "magnesium" ;
	if _N_=18 then varname = "fibers" ;
	if _N_=19 then varname = "potassium" ;
	run;
