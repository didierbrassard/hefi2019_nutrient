...
/* TO DO: indicate location of repository */
	%let path = C:/Users/DIBRA22/Documents/hefi2019_nutrients/ ;

/* TO DO: indicate location of cchs files (could be the same as <path>) */
	%let pathCCHS = C:/Users/DIBRA22/Documents/hefi2019_nutrients/Data/Raw/CCHS_Nutrition_2015_PUMF/;
...
/* AUTOMATIC: library to output files in project folder */
	...
	libname proj "&path./";
	libname _fmt "&path./Data/";
	libname fmt "&path./Data/Processed/";
	libname temp "&path./Temp/"; * temporary (large) files are saved there;
...
/* 1) import file using proc import */
	...
	PROC IMPORT DATAFILE="&path./Data/Raw/HEFI2019_foodcat_2022-03.xlsx"
		DBMS=XLSX
...
/* 2) prepare classification to merge with other cchs files */
 ...
 /*************************************************************************/
	...
	/* note: assuming the file is at the <path> location,
		in a folder named <Data/Raw/> */

/* 1) import file using proc import */	
	options validvarname=v7;
	PROC IMPORT DATAFILE="&path./Data/Raw/nutrients-1173784-supplementary.xlsx"
		DBMS=XLSX
...
/* 1) Import HS_NCI data, save to fmt folder */
	data FMT.HS_NCI  (rename=(ADMFW=r24_weekend ADMDD=r24_day ADM_MOI=r24_month 
		R24DCNT=r24_nfoods GEO_PRV=province DHH_SEX=sex DHH_AGE=age DHHDDRI=drig 
		EDUG21=education MHWGHTM=height MHWGWTK=bodyweight FSDDWTG=foodwgt 
		FSDDEKC=energy FSDDSOD=sodium FSDDPOT=potassium FSDDFAS=sfa FSDDFAM=mufa 
		FSDDFAP=pufa FSDDFI=fibers FSDDPRO=proteins FSDDCAL=calcium FSDDDMG=vit_d 
		FSDDIRO=iron FSDDZIN=zinc FSDDMAG=magnesium FSDDB6=vit_b6 FSDDB12=vit_b12 
		FSDDDFE=folate FSDDRAE=vit_a));
	...
	* keep only respondents aged 2 years and older (i.e., target population of Canada`s food guide);
	...
	keep /* id */ ADM_RNO WTS_P
		 ...
		 /*characteristics */ GEO_PRV DHH_AGE DHH_SEX DHHDDRI EDUG21 SMOKING ADMFW MHWGHTM MHWGWTK
		 /* dietary intakes*/ FSDDWTG FSDDEKC FSDDSOD FSDDPOT FSDDFAT FSDDFAS FSDDFAM FSDDFAP
		                      FSDDFI FSDDPRO FSDDCAL FSDDDMG FSDDIRO FSDDZIN FSDDB6 FSDDB12 
		                      FSDDDFE FSDDMAG FSDDRAE;
	run;
...
/* Merge all data into one by subject and recall number */
	data fmt.intake_per24hr(drop=i);
		merge food_sum_t
			  bev_sum_t
			  freesug_sum 
			  fmt.hs_nci(in=hs keep=ADM_RNO SUPPID 
				energy sfa mufa pufa sodium proteins
				calcium vit_d iron zinc vit_b6 vit_b12 vit_a
				potassium fibers folate magnesium )
				;
		by ADM_RNO SUPPID;
...
/* 1) Define a macro variable for all dietary constituents of the HEFI-2019 
	and nutrients of interest */
	
	%let hefi2019_vars = vf wg rg pfab pfpb otherfoods water milk plantbev otherbevs sfa mufa pufa freesugars sodium energy;
	%let nutrients = proteins calcium vit_d iron zinc vit_b6 vit_b12 magnesium fibers folate potassium vit_a;
...
/* 3) Descriptive statistics */
	...
	format &hefi2019_vars &nutrients zerofmt. ; 
	table &hefi2019_vars &nutrients ;
