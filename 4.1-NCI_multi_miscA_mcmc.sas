...
/* 4) Look at intakes on a given day */
	...
	%let daily_list  = vf otherfoods pfab water mufa pufa sfa freesugars sodium energy iron zinc vit_b6 vit_b12;
...
run;
 ...
 /********************************************************************/
 /*   Dietary constituents #16 - iron: min. amount and lambda   */
 /********************************************************************/
...
data _temp16;
set inboxcox(keep=adm_rno bsw0 iron seq2 r24_weekend agec_71plus );
if (iron > 0) then output;
run;
...
run;
 ...
 /********************************************************************/
 /*   Dietary constituents #17 - zinc: min. amount and lambda   */
 /********************************************************************/
...
data _temp17;
set inboxcox(keep=adm_rno bsw0 zinc seq2 r24_weekend agec_71plus );
if (zinc > 0) then output;
run;
...
run;
 ...
 /********************************************************************/
 /*   Dietary constituents #18 - vit_b6: min. amount and lambda   */
 /********************************************************************/
...
data _temp18;
set inboxcox(keep=adm_rno bsw0 vit_b6 seq2 r24_weekend agec_71plus );
if (vit_b6 > 0) then output;
run;
...
run;
 ...
 /********************************************************************/
 /*   Dietary constituents #19 - vit_b12: min. amount and lambda   */
 /********************************************************************/
...
data _temp19;
set inboxcox(keep=adm_rno bsw0 vit_b12 seq2 r24_weekend agec_71plus );
if (vit_b12 > 0) then output;
run;
...
* Call the <std_cov_boxcox24hr_conday_minamt> macro to standardize data and continuous covariates, if any; 
 %std_cov_boxcox24hr_conday_minamt(
  ...
  rec24hr_daily_vars         = vf otherfoods pfab water mufa pufa sfa freesugars sodium energy iron zinc vit_b6 vit_b12, 
...
* Call the <multivar_mcmc> macro to fit the measurement error model; 
 ...
 stdbc_freesugars  stdbc_sodium  stdbc_energy  stdbc_iron  stdbc_zinc  stdbc_vit_b6  stdbc_vit_b12, 
...
* Save lambdas (Box-Cox transformation lambda values) and minimum amount data ;
	...
	* indicate replicate number, current stratum value, variable name;
	...
	if _N_=16 then varname = "iron" ;
	if _N_=17 then varname = "zinc" ;
	if _N_=18 then varname = "vit_b6" ;
	if _N_=19 then varname = "vit_b12" ;
	run;
