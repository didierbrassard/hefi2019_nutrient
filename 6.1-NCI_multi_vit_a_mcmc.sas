...
/* 4) Look at intakes on a given day */
	...
	%let daily_list  = vf otherfoods pfab water mufa pufa sfa freesugars sodium energy vit_a ;
...
run;
 ...
 /********************************************************************/
 /*   Dietary constituents #16 - vit_a: min. amount and lambda       */
 /********************************************************************/
...
data _temp16;
set inboxcox(keep=adm_rno bsw0 vit_a seq2 r24_weekend agec_71plus );
if (vit_a > 0) then output;
run;
 ...
 * Call the <std_cov_boxcox24hr_conday_minamt> macro to standardize data and continuous covariates, if any; 
 %std_cov_boxcox24hr_conday_minamt(
  ...
  rec24hr_daily_vars         = vf otherfoods pfab water mufa pufa sfa freesugars sodium energy vit_a, 
...
* Call the <multivar_mcmc> macro to fit the measurement error model; 
 ...
 stdbc_freesugars  stdbc_sodium  stdbc_energy  stdbc_vit_a, 
