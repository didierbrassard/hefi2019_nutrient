...
/* 0) Import sampling weights required for bodyweight estimation */
	data work.BSW_hw;
		%let datafid = "&pathCCHS./Bootstrap/Data_Donnee/b5_hw.txt";
		%include "&pathCCHS./Bootstrap/SAS_SPSS/b5_HW_i.sas";
	run;
	...
	/* note: b5_hw must be used to consider bodyweight in the modelling */
...
/* 1) Combine dietary intakes of the first recall with sociodemo data */
	data intake_and_sociodeom(drop=nonzero_energy);
		merge fmtdata.intake_per24hr(in=a)
			  fmtdata.hs_nci(in=b keep=adm_rno suppid wts_p r24_weekend drig age sex bodyweight height);
		by adm_rno suppid;
	* remove 24-h recall with 0 energy intake ;
		if energy=0 then delete ;
	* keep adm_rno present in both data ;
...
/* 2) Re-code covariates as dummy variables, select final sample and output data*/
	data preNCI;
		merge intake_and_sociodeom (in=intake rename=(r24_weekend=_r24_weekend))
			  BSW_hw (in=boot drop=BSW:) ;
		by adm_rno ;
	...
	* Change variable name for sampling weights to be consistent with bootstrap weights ;
	rename FWGT =bsw0; * using the sampling weight variable unique to height/weight data;
		
	* Filter to keep only respondents 65 years or older;
	if age <65 then delete ;
	if missing(bodyweight) then delete;
	
	if intake and boot then output;
	run;

	/* note: sample size of respondents 65y+ for both 24-h recall without missing bodyweight = 3,818 */
	
/* 3) Confirm age categories recoding */
	
	proc freq data=preNCI;
	table drig *suppid drig * agec: sex: ;
	run;

/* 4) Look at intakes on a given day */

	/* 4.0) define dietary constituents */
	%let daily_list  = vf otherfoods pfab water mufa pufa sfa freesugars sodium energy proteins ;
	...
	class suppid; 
	var &daily_list. &episo_list. bodyweight height;
	weight bsw0 ;
	run;
...
data _temp1;
set inboxcox(keep=adm_rno bsw0 wg seq2 r24_weekend bodyweight agec_71plus );
if (wg > 0) then output;
run;
...
* 2) Call the <boxcox_survey> macro to find best normal transformation ;
 %boxcox_survey(
  ...
  subject = adm_rno, 
  var     = wg,  
  covars  = seq2 r24_weekend bodyweight agec_71plus ,  
  weight  = bsw0,  
  print   = N, 
  plot    = N, 
  ntitle  = 4 ); 
...
data _temp2;
set inboxcox(keep=adm_rno bsw0 pfpb seq2 r24_weekend bodyweight agec_71plus );
...
data _temp3;
set inboxcox(keep=adm_rno bsw0 otherbevs seq2 r24_weekend bodyweight agec_71plus );
...
data _temp4;
set inboxcox(keep=adm_rno bsw0 milk seq2 r24_weekend bodyweight agec_71plus );
...
data _temp5;
set inboxcox(keep=adm_rno bsw0 rg seq2 r24_weekend bodyweight agec_71plus );
if (rg > 0) then output;
run;
...
data _temp6;
set inboxcox(keep=adm_rno bsw0 vf seq2 r24_weekend bodyweight agec_71plus );
if (vf > 0) then output;
run;
...
data _temp7;
set inboxcox(keep=adm_rno bsw0 otherfoods seq2 r24_weekend bodyweight agec_71plus );
...
data _temp8;
set inboxcox(keep=adm_rno bsw0 pfab seq2 r24_weekend bodyweight agec_71plus );
...
data _temp9;
set inboxcox(keep=adm_rno bsw0 water seq2 r24_weekend bodyweight agec_71plus );
...
data _temp10;
set inboxcox(keep=adm_rno bsw0 mufa seq2 r24_weekend bodyweight agec_71plus );
...
data _temp11;
set inboxcox(keep=adm_rno bsw0 pufa seq2 r24_weekend bodyweight agec_71plus );
...
data _temp12;
set inboxcox(keep=adm_rno bsw0 sfa seq2 r24_weekend bodyweight agec_71plus );
...
data _temp13;
set inboxcox(keep=adm_rno bsw0 freesugars seq2 r24_weekend bodyweight agec_71plus );
...
data _temp14;
set inboxcox(keep=adm_rno bsw0 sodium seq2 r24_weekend bodyweight agec_71plus );
...
data _temp15;
set inboxcox(keep=adm_rno bsw0 energy seq2 r24_weekend bodyweight agec_71plus );
...
run;
 ...
 /********************************************************************/
 /*   Dietary constituents #16 - proteins: min. amount and lambda   */
 /********************************************************************/
...
data _temp16;
set inboxcox(keep=adm_rno bsw0 proteins seq2 r24_weekend bodyweight agec_71plus );
if (proteins > 0) then output;
run;
...
run;
 ...
 %std_cov_boxcox24hr_conday_minamt(
  data                       = _tempstratum1, 
  prestand_continuous_covars = bodyweight , 
  rec24hr_epis_vars          = wg pfpb otherbevs milk rg, 
  rec24hr_daily_vars         = vf otherfoods pfab water mufa pufa sfa freesugars sodium energy proteins, 
...
/*************************************************************************/
 ...
 stdbc_freesugars  stdbc_sodium  stdbc_energy  stdbc_proteins, 
  covars_epis_prob            = constant1 seq2 r24_weekend agec_71plus std_bodyweight  , 
  covars_epis_amt             = constant1 seq2 r24_weekend agec_71plus std_bodyweight  , 
  covars_daily_amt            = constant1 seq2 r24_weekend agec_71plus std_bodyweight  , 
  set_seed_mcmc               = 42941, 
  ...
  optional_iml_store_names    = constant1 seq2 r24_weekend agec_71plus std_bodyweight  tran_paramindex tran_lambda tran_center 
 tran_scale minamount 
  ); 
...
if _N_=16 then varname = "proteins" ;
run;
...
proc sort data=_tempstratum1 nodupkey out=optional_input_data(keep= adm_rno sex bsw0 
 agec_71plus bodyweight sex drig bodyweight);
by adm_rno ;
run;
 ...
 %multivar_distrib(
  ...
  t_weightavg_covariates_list1    = constant1 constant0 constant0 agec_71plus std_bodyweight  ,  
  t_weightavg_covariates_list2    = constant1 constant0 constant1 agec_71plus std_bodyweight , 
  ...
  additional_output_var_list      = sex sex drig bodyweight , 
...
/*************************************************************************/
 NCI_MCMC_SURVEILLANCE_SVY Stratum selection based on STRATA=sex (2/2)
/*************************************************************************/
